...
void *multMatriz (void *tid) {
    ...
    for (; i < linhasA; i += nthreads) {
        ...
        for (j = 0; j < colunasB; j++) {
            matC[i] += matA[i * colunasA + j] * matB[j * colunasA + i];
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    if(argc < 5) {
        fprint(stderr, "Digite: %s <arquivo matriz A> <arquivo matriz B> <arquivo matriz C (saída)> <número de threads (1 a %d)> \n", argv[0], numCPU);
        return 1;
    }
    ...
    printf("Tempo necessário para criação das threads e multiplicação das matrizes: %f", fim - inicio);
...
printf("Tempo para finalização: %f", fim - inicio);
